...
#------------------------------------------#
# 1. Display menu allowing the user to choose: 'Add CD', 'Display Current Inventory', 'Save Inventory to file', 'Delete Data from Inventory file' and 'exit'
strChoice = ''
dicRow = []
lstTblData = []
lstRow = []
...
# 3. Add data to the dictionarie each time the user wants to add data
    if Choice.lower() == 'a':
...
# 4. Save the data to a text file MyData.txt if the user chooses so
    elif Choice.lower() == 's':
...
# 5. Read the Data from MyData.txt file
    elif Choice.lower() == 'r':
...
# 6. Delete an entry from the file
#open the file
...
#update the data file
        ...
        lstTblData = []
        delChoice = None
# 7. Check if the User have choose something else from the List
    else:
        print('Wrong choice')
        print('-------------------------')
